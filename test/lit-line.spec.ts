...
const creationTests: CreationTest[] = [
  ...
  },
  {
    ...
    it: {
      ...
      renders: [
        [
          {time: CHART_WIDTH / 2, value: CHART_HEIGHT / 2}
        ]
      ]
    }
  },
  {
    when: {
      ...
      data: [{points: [{time: 0, value: 1}, {time: 1, value: 2}]}]
    },
    it: {
  ...
  },
  {
    when: {
      ...
      data: [
        ...
        {points: [{time: 2, value: 2}, {time: 3, value: 3}]}
      ]
    },
    it:{
      ...
      renders: [
        [
          {time: CHART_MARGIN, value: CHART_HEIGHT - CHART_MARGIN},
          {time: CHART_MARGIN + (CHART_WIDTH - 2 * CHART_MARGIN) / 3, value: CHART_HEIGHT - (CHART_MARGIN + (CHART_HEIGHT - 2 * CHART_MARGIN) / 3)}
        ],
        [
          {time: CHART_MARGIN + 2 * (CHART_WIDTH - 2 * CHART_MARGIN) / 3, value: CHART_HEIGHT - (CHART_MARGIN + 2 * (CHART_HEIGHT - 2 * CHART_MARGIN) / 3)},
          {time: CHART_MARGIN + 3 * (CHART_WIDTH - 2 * CHART_MARGIN) / 3, value: CHART_HEIGHT - (CHART_MARGIN + 3 * (CHART_HEIGHT - 2 * CHART_MARGIN) / 3)}
        ]
      ]
    }
  },
  {
    when: {
      ...
      data: [
        {unit: 'usd', points: [{time: 0, value: 1}, {time: 1, value: 2}]},
        {unit: 'eur', points: [{time: 0, value: 10}, {time: 1, value: 20}]}
      ]
    },
    it: {
...
creationTests.forEach(test => {
  describe(`[lit-line] when ${test.when.desc}`, async () => {
        ...
        describe(`checking serie #${serieId}`, () => {
          ...
          it(`renders valid ranges`, async () => {
            ...
            serie.forEach((slot, slotId) => {
              ...
              expect(rangeElement?.getAttribute('height')).toEqual(`${slot.range ? slot.range.max - slot.range.min : 0}`);
